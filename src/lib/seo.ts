...
const BASE_URL = process.env.NEXT_PUBLIC_SITE_URL || 'https://waltenne.github.io/aprendi/';
// Prefixo público (basePath) usado quando hospedado em um subpath (ex: GitHub Pages)
const BASE_PATH = process.env.NEXT_PUBLIC_BASE_PATH ?? (process.env.GITHUB_PAGES === 'true' && process.env.GITHUB_REPOSITORY_NAME ? `/${process.env.GITHUB_REPOSITORY_NAME}` : '');
...
export function getCourseMetadata(course: CourseWithInstructor): Metadata {
  ...
  const ogImage = `${BASE_URL}${BASE_PATH || ''}/api/og?title=${encodeURIComponent(title)}&icon=${encodeURIComponent(course.icon || '')}`;
...
export function getInstructorMetadata(instructor: Instructor): Metadata {
  ...
  const avatarUrl = instructor.avatar
    ? (instructor.avatar.startsWith('http') ? instructor.avatar : `${BASE_PATH || ''}${instructor.avatar}`)
    : undefined;
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      siteName: SITE_NAME,
      images: avatarUrl ? [{ url: avatarUrl, alt: instructor.name }] : undefined,
    },
    twitter: {
      card: 'summary',
      title: `${title} - Instrutor`,
...
export function getOrganizationJsonLd() {
  return {
    ...
    name: SITE_NAME,
    url: BASE_URL,
    logo: `${BASE_URL}${BASE_PATH || ''}/logo.png`,
    description: DEFAULT_DESCRIPTION,
    sameAs: [],
  };
}
    ...
    openGraph: {
      ...
      images: instructor.avatar 
        ? [{ url: instructor.avatar, alt: instructor.name }]
          url: `${BASE_URL}${BASE_PATH || ''}/api/og?title=${encodeURIComponent(title)}&icon=${encodeURIComponent(course.icon || '')}`,
    },
    twitter: {
      card: 'summary',
      title: `${title} - Instrutor`,
